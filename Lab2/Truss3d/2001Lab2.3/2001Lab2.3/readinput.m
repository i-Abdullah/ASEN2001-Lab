...
while line > 0
    ...
    switch inpblk
        ...
        case 1 % read number of joints, bars, reactions, and loads
            ...
            % initialize arrays
            joints       = zeros(numjoints,3); %changed to 3 for 3D
            connectivity = zeros(numbars,2);
            reacjoints   = zeros(numreact,1); %changed to 3 for 3D
            reacvecs     = zeros(numreact,3); 
            loadjoints   = zeros(numloads,1);
            loadvecs     = zeros(numloads,3); %changed to 3 for 3D
        ...
        case 2 % read coordinates of joints
            ...
            tmp=sscanf(line,'%d%e%e%e'); %changed to 3*%e for 3D
            ...
            joints(jointid,:)=tmp(2:4); %changed to 2:4 for 3D
        ...
        case 4 % read reation force information
            ...
            uvec=tmp(2:4); %changed to 2:4 for 3D
            uvec=uvec/norm(uvec);
        ...
        case 5 % read external load information
            ...
            frcvec=tmp(2:4); %changed to 2:4 for 3D
