function [barforces,reacforces]=FAA(joints,connectivity,reacjoints,reacvecs,loadjoints,loadvecs)
...
% number of equations
numeqns = 3 * numjoints; %changed to 3 for 3D
...
for i=1:numjoints
   ...
   for ib=1:length(ibar)
       ...
       %Actually, for 3D, get coordinates for joints "i", "j", and "k" of
       %bar "barid"
       joint_i = joints(i,:);
       if ijt(ib) == 1
       ...
       Amat([idx idy idz],barid)=uvec; %changed to idx idy idz for 3D
   end
end
...
for i=1:numreact
    ...
    % add unit vector into Amat
    Amat([idx idy idz],numbars+i)=reacvecs(i,:); %changed to idx idy idz for 3D
end
...
for i=1:numloads
    ...
    bvec([idx idy idz])=-loadvecs(i,:); %changed to idx idy idz for 3D
end
